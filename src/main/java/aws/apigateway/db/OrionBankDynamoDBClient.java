...
/**
 * Client for accessing OrionBankDynamoDB. All service calls made using this client are blocking, and will not return
...
class OrionBankDynamoDBClient implements OrionBankDynamoDB {
    ...
    /**
     * Constructs a new client to invoke service methods on OrionBankDynamoDB using the specified parameters.
     *
     * <p>
    ...
    public PostDynamodbResult postDynamodb(PostDynamodbRequest postDynamodbRequest) {
        HttpResponseHandler<PostDynamodbResult> responseHandler = protocolFactory.createResponseHandler(new JsonOperationMetadata().withPayloadJson(true)
                .withHasStreamingSuccessResponse(false), new PostDynamodbResultJsonUnmarshaller());

        HttpResponseHandler<SdkBaseException> errorResponseHandler = createErrorResponseHandler();

        return clientHandler.execute(new ClientExecutionParams<PostDynamodbRequest, PostDynamodbResult>()
                .withMarshaller(new PostDynamodbRequestProtocolMarshaller(protocolFactory)).withResponseHandler(responseHandler)
                .withErrorResponseHandler(errorResponseHandler).withInput(postDynamodbRequest));
    }
